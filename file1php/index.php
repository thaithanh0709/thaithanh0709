		...
		<div class="element header1">
			...
			<span class="menu">
				<ul>
					...
					<li><a href="index.php">Giày Bóng Đá</a></li>
...
<?php
	if (isset($_POST['btntimkiem'])) {
		...
		$stm = $o->prepare($query);
		$sql = $o->query($query);
		// $sql = "SELECT COUNT(*) FROM product WHERE WHERE product.name LIKE '%$datatk%'";
		// $count = $sql->rowCount();
		if ($stm->execute()) {
			while ($row = $stm->fetch(PDO::FETCH_ASSOC)) {?>
				<!-- <tr><td><?php echo 'Kết quả tìm kiếm : " '.$datatk.' co ' .$count. ' san pham '; ?></td></tr><br> -->
				<tr>
